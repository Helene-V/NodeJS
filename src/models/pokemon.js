  ...
  return sequelize.define('Pokemon', {
    ...
    types: {
      ...
      validate: {
        isTypesValid(value) {
          if(!value) {
            throw new Error('Un pokemon doit avoir au moins u ntype.')
          }
          ...
          value.split(',').forEach(type => {
            if(!valideTypes.includes(type)) {
              throw new Error(`Le type d'un pokémon doit appartenir à la liste suivante : ${valideTypes}`)
            }
          })
        }
      }
...
//Sequelize va créer la table "pokemons" au pluriel automatiquement
// 1 modèle Sequelize = 1 table dans la DB
